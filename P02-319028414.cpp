...
void CrearFiguras()
{
	...
	int numVerticesC = sizeof(pos_letra_C) / (3 * sizeof(GLfloat));
	GLfloat* letra_C = new GLfloat[numVerticesC * 6];
	for (int i = 0; i < numVerticesC; i++) {
		// Copiamos las coordenadas de posición
		letra_C[i * 6 + 0] = pos_letra_C[i * 3 + 0];
		letra_C[i * 6 + 1] = pos_letra_C[i * 3 + 1];
		letra_C[i * 6 + 2] = pos_letra_C[i * 3 + 2];
		// Asignamos colores aleatorios
		letra_C[i * 6 + 3] = randomColor();
		letra_C[i * 6 + 4] = randomColor();
		letra_C[i * 6 + 5] = randomColor();
	}

	MeshColor* meshLetra_C = new MeshColor();
	meshLetra_C->CreateMeshColor(letra_C, numVerticesC * 6);
	meshColorList.push_back(meshLetra_C);

	// Vértices de la letra E
	GLfloat pos_letra_E[] = {
	...
	int numVerticesE = sizeof(pos_letra_E) / (3 * sizeof(GLfloat));
	GLfloat* letra_E = new GLfloat[numVerticesE * 6];
	for (int i = 0; i < numVerticesE; i++) {
		// Copiamos las coordenadas de posición
		letra_E[i * 6 + 0] = pos_letra_E[i * 3 + 0];
		letra_E[i * 6 + 1] = pos_letra_E[i * 3 + 1];
		letra_E[i * 6 + 2] = pos_letra_E[i * 3 + 2];
		// Asignamos colores aleatorios
		letra_E[i * 6 + 3] = randomColor();
		letra_E[i * 6 + 4] = randomColor();
		letra_E[i * 6 + 5] = randomColor();
	}

	MeshColor* meshLetra_E = new MeshColor();
	meshLetra_E->CreateMeshColor(letra_E, numVerticesE * 6);
	meshColorList.push_back(meshLetra_E);
